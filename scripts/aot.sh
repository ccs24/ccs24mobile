...
if [ $TRAVIS_BRANCH == 'integration' ] || [ $TRAVIS_BRANCH == 'master' ] || [ $TRAVIS_BRANCH == 'desktop' ] || [ -z $TRAVIS_BRANCH ] ; then
    ...
    if [ $TRAVIS_BRANCH == 'master' ] && [ ! -z $GIT_TOKEN ] ; then
        ...
        pushd ../../moodle-local_ccs24mobileapp
        sed -ie "s/release[ ]*=[ ]*'[^']*';/release = '$version';/1" version.php
        sed -ie "s/version[ ]*=[ ]*[0-9]*;/version = $date;/1" version.php
        rm version.phpe
        git remote set-url origin https://$GIT_TOKEN@github.com/moodlehq/moodle-local_ccs24mobileapp.git
        git fetch -q origin
        git add .
...
if [ ! -z $GIT_ORG ] && [ ! -z $GIT_TOKEN ] ; then
    gitfolder=${PWD##*/}
    git clone --depth 1 --no-single-branch https://github.com/$GIT_ORG/ccs24mobile-phonegapbuild.git ../pgb
    pushd ../pgb
    ...
    git push https://$GIT_TOKEN@github.com/$GIT_ORG/ccs24mobile-phonegapbuild.git
    popd
fi
