...
#Checks if a key exists on the original local_ccs24mobileapp.php
function exists_in_mobile {
    file='local_ccs24mobileapp'
...
function find_matches {
    ...
    print_message "No match found for $key add it to local_ccs24mobileapp"
    save_key $key "local_ccs24mobileapp"
}
...
function find_single_matches {
    ...
    do_match " = \'$value\'"
    if [ ! -z $filematch ] && [ $filematch != 'local_ccs24mobileapp' ]; then
        case=2
...
function guess_file {
    ...
    # Not found in file, try in local_ccs24mobileapp
    if [ $found == 0 ]; then
        exists_in_mobile
    fi
...
function find_better_file {
    ...
    if [ $currentFile == 'local_ccs24mobileapp' ]; then
        exists_in_mobile
    else
    ...
    if [ $found == 0 ]; then
        ...
        if [ $currentFile != 'local_ccs24mobileapp' ]; then
            ...
            CPY [$currentStr,$currentFile],[$key,local_ccs24mobileapp]"
            save_key $key "local_ccs24mobileapp"
        fi
    fi
}
