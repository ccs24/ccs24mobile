...
function build_lang($lang, $keys, $total) {
    ...
    foreach ($keys as $key => $value) {
        ...
        if ($value->file != 'local_ccs24mobileapp') {
    ...
    if ($lang == 'en') {
        generate_local_ccs24mobileapp($keys, $translations);
 ...
 */
function generate_local_ccs24mobileapp($keys, $translations) {
    echo "Generate local_ccs24mobileapp.\n";
    $string = '<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
...
Prevent phone from sleeping - So you can receive push notifications anytime\';'."\n";
    foreach ($keys as $key => $value) {
        if (isset($translations[$key]) && $value->file == 'local_ccs24mobileapp') {
            $string .= '$string[\''.$key.'\'] = \''.str_replace("'", "\'", $translations[$key]).'\';'."\n";
        }
    }
    $string .= '$string[\'pluginname\'] = \'Moodle Mobile language strings\';'."\n";

    file_put_contents('../../moodle-local_ccs24mobileapp/lang/en/local_ccs24mobileapp.php', $string."\n");
}
